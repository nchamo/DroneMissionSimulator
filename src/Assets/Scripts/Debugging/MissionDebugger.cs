...
public class MissionDebugger {
    ...
    public MissionDebugger(CameraDefinition cameraDefinition, SurveyArea surveyArea, FlightPlan flightPlan, Transform droneTransform) {
    ...
    public void DrawDebug(int pointInPlan) { 
        for (int i = 0; i < pointInPlan; i++) {
            DrawCompleted(flightPlan.flightSteps[i]);
        }
        if (pointInPlan > 0) {
            DrawCurrent(Mathf.Min(pointInPlan, flightPlan.flightSteps.Count - 1));
        }
        DrawArea();
    }
    ...
    private void DrawCurrent(int pointInPlan) {
        FlightStep current = flightPlan.flightSteps[pointInPlan];
    ...
    private void DrawCompleted(FlightStep flightStep) {
        (Vector3, Vector3, Vector3, Vector3) box = CalculateBoxFromPositionAndRotation(flightStep.dronePosition, flightStep.droneRotation);
